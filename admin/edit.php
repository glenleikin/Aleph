...
<section class="services gradient">
      ...
      <div class="container">
        ...
        <?php
            ...
            $info = recoverBook($idLibro);
            if(isset($_POST['send'])){
                $answer = bookEdit($_POST);
            ...
            <form action="#" method="post" enctype="multipart/form-data">
                <div class="form-group">
                    <label for="inputName">Nombre</label>
                     <input type="text" class="form-control" id="inputName" name="name" value="<?= $info['Nombre']?>">
                </div>
                <br>
                <div class="form-group">
                    <label for="inputName">Genero</label>
                     <input type="text" class="form-control" id="inputName" name="genre" value="<?= $info['Genero']?>">
                </div>
                <br>
                <div class="form-group">
                    <label for="inputName">Anio</label>
                     <input type="text" class="form-control" id="inputName" name="year" value = "<?= $info['Anio']?>">
                </div>
                <div class="form-group">
                    ...
                    <?php
                        ...
                        foreach ($row as $value) {
                            if ($value['idAutor'] == $info['Autor']) {
                        ?>
                                <option value="<?= $value['idAutor'] ?>" selected><?= $value['Nombre'] ?></option>
                            <?php
                            } else {
                            ?>
                ...
                <div class="form-group">
                    <label for="inputName">Imagen</label>
                      <input type="hidden" name="imagenActual" value="<?= $info['Imagen'] ?>">
                      <input type="hidden" name="idLibro" value="<?= $info['idLibro'] ?>">
                      <img src="../images/books/<?= $info['Imagen'] ?>" style="width:130px">
                      <input type="file" name="imagen">
                </div>
                <br>
                <br>
                <button type="submit" class="btn btn-dark" name="send">Guardar cambios</button>
            </form>
          </div>
